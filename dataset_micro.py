...
read_dir = 'backup_datasets/ModelNet10/'
micro_file_path = 'mesh_info.txt'
path_prefix = 'backup_datasets/ModelNet10_clean_faces_removed/'
micro_prefix = 'ModelNet10_micro/'
...
def fill_from_file(file,write_dir):
    ...
    with open(file, 'r') as f:
        ...
        faces = [[int(s) for s in f.readline().strip().split(' ')][0:] for i_face in range(n_faces)]
    vs = np.asarray(vs)
...
def create_micro_dataset():
    global micro_file_path, micro_prefix

    with open(micro_file_path, 'r') as f:
        lines = f.readlines()
    # print(len(lines))
    ...
    for line in lines:
        ...
        dst = micro_prefix+file_path.split('ModelNet10/')[1]
        # print(file_path)
        # print(dst)
